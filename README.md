...
4. Pass the mandatory risk-adaptive gate with `feature-workflow:feature-qa-review`
...
- Subagent-Driven batches use temporary Superpowers-compatible plans and retain native implementer, per-task review, fix-loop, and final batch-review behavior; control returns to `feature-implementing` instead of running branch finishing.
- One shared branch/worktree and conditional Docker/Compose preflight is reused across child execution while the environment remains stable.
- Z98 records task results, commits, verification coverage, captured evidence, reuse, invalidation, and phase approvals for both modes.
- A tracker child item is done only after the workflow validates the returned commit SHA on the epic branch and focused verification was reported.
- Integrated targeted and required regression verification run once at the final runtime-affecting HEAD; later documentation-only commits do not invalidate that evidence.
...
- QA binds review and verification evidence to full commit SHAs and always runs one integrated independent reviewer.
- QA adds security or domain specialists only when concrete risk signals require them.
- Candidate findings become `Blocking Confirmed`, `Advisory`, or `Dismissed`; only unresolved confirmed blockers prevent PASS.
- Approved blocker fixes are handled in one wave and covered by scoped fix-delta review; broader fixes trigger risk reclassification rather than an automatic full-profile rerun.
- QA reads Z98 and reuses valid evidence from the reviewed commit or a verified runtime ancestor until affected code, environment, or repository/risk requirements invalidate it.
- A `PASS` still requires explicit user acceptance before QA can invoke finishing.
- Finishing checks the accepted Z06 and reviewed commit before editing documentation.
- Finishing owns documentation consistency and change recording; it does not repeat QA's code-review profiles or still-valid implementation tests.
- Any required non-documentation change stops finishing and routes the resulting implementation delta back through risk-adaptive QA.
...
- `feature-implementing` wraps superpowers `subagent-driven-development` or `executing-plans` behind phase-scoped execution.
